...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onDelete, onUpdate }) => {
  ...
  return (
    <div className={`group border-b border-slate-100 last:border-0 hover:bg-slate-50/80 transition-colors ${isExpanded ? 'bg-slate-50 border-b-slate-200' : 'bg-white'}`}>
      ...
      >
        ...
        <div
          ...
          className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 cursor-pointer transition-all ${isCompleted ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 hover:border-indigo-500 bg-white'}`}
        >
          {isCompleted && (
        ...
        <div className="flex-1 min-w-0 flex items-center gap-3">
          <span className={`text-sm font-medium truncate ${isCompleted ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
            {task.title}
          </span>
          {task.description && (
            <svg className="w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h7" /></svg>
          )}
        </div>
        ...
        <div className="hidden md:flex items-center gap-3 shrink-0">
          ...
          {task.value && (
            <span className="text-xs font-mono text-slate-600 bg-slate-100 px-1.5 rounded">{task.value}</span>
          )}
          ...
          {task.endDate && (
            <span className={`text-xs ${new Date(task.endDate) < new Date() && !isCompleted ? 'text-red-500 font-medium' : 'text-slate-400'}`}>
        ...
        <div className="px-12 pb-6 pt-2 cursor-default">
          ...
          {!isEditing ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-6 text-sm text-slate-600">
                <div className="flex-1">
                  <h4 className="text-xs font-semibold text-slate-400 uppercase mb-1">Descrição</h4>
                  <p className="whitespace-pre-wrap">{task.description || "Sem descrição."}</p>
                </div>
                {(task.pagamento?.status !== 'Não aplicável' && task.pagamento) && (
                  <div className="w-1/3 bg-slate-100 rounded p-3 text-xs">
                    <h4 className="font-semibold text-slate-700 mb-2">Financeiro</h4>
                    <div className="flex justify-between mb-1">
                      <span>Status:</span>
                      <span className={task.pagamento.status === 'Pago' ? 'text-emerald-600' : 'text-amber-600'}>{task.pagamento.status}</span>
                    </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button onClick={() => setIsEditing(true)} className="px-3 py-1.5 text-xs font-medium text-slate-700 bg-white border border-slate-300 rounded hover:bg-slate-50 transition">
                  Editar Tarefa
                </button>
                <button onClick={() => { if (window.confirm('Excluir?')) onDelete(task.id); }} className="px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 rounded transition">
                  Excluir
                </button>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm space-y-4">
              {/* Edit Form */}
              <input
                className="w-full text-base font-medium border-0 border-b border-slate-200 focus:ring-0 px-0 pb-2 placeholder:text-slate-300"
              ...
              <textarea
                className="w-full text-sm text-slate-600 border-0 bg-slate-50 rounded p-2 focus:ring-1 focus:ring-indigo-500 min-h-[80px]"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <select
                    className="w-full text-xs border-slate-200 rounded focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                <button onClick={handleCancel} className="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-800">Cancelar</button>
